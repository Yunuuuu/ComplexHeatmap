...
anno <- anno_horizon(lt, which = "row", gp = gpar(
...
if (0) {
  ...
  grid.text <- function(text, x = 0.5, y = 0.5, gp = gpar(), rot = 0, default.units = "npc", just = "center") {
    ...
    if (is.character(just)) {
      just2[1] <- switch(just[1],
        "center" = 0.5,
        "left" = 0,
        "right" = 1
      )
      just2[2] <- switch(just[2],
        "center" = 0.5,
        "bottom" = 0,
        "top" = 1
      )
    }
    gb <- richtext_grob(text,
      x = x, y = y, gp = gpar(fontsize = 10), box_gp = gpar(col = "black"),
...
anno <- anno_simple(1:10,
  pch = 1:10, pt_gp = gpar(col = rep(c(1, 2), each = 5)),
...
anno <- anno_points(1:10,
  pch = 1:10, gp = gpar(col = rep(c(1, 2), each = 5)),
  size = unit(1:10, "mm")
)
draw(anno, index, test = "a numeric vector")
anno <- anno_points(1:10,
...
anno <- anno_boxplot(m,
  pch = 1:10, gp = gpar(fill = rep(c(1, 2), each = 5)),
  size = unit(1:10, "mm"), height = unit(4, "cm")
)
draw(anno, index, test = "a numeric vector")
anno <- anno_boxplot(t(m),
  pch = 1:10, gp = gpar(fill = rep(c(1, 2), each = 5)),
...
m <- matrix(rnorm(1000), nc = 10)
lt <- apply(m, 2, function(x) data.frame(density(x)[c("x", "y")]))
anno <- anno_joyplot(lt,
  gp = gpar(fill = rep(c(1, 2), each = 5)),
...
anno <- anno_zoom(
  align_to = fa, which = "row", panel_fun = panel_fun,
  gap = unit(1, "cm")
)
...
anno2 <- anno_zoom(
  align_to = subgroup, which = "row", panel_fun = panel_fun2,
  gap = unit(1, "cm"), width = unit(3, "cm"), side = "left"
)

draw(Heatmap(m,
  right_annotation = rowAnnotation(
    subgroup = subgroup, foo = anno,
...
row_split <- c(
  rep("a", 10), rep("b", 5), rep("c", 2), rep("d", 3),
