...
lgd <- Legend(
  labels = month.name[1:6], title = "foo", legend_gp = gpar(fill = 1:6),
...
lgd <- Legend(
  at = 1:6, legend_gp = gpar(fill = 1:6), grid_height = unit(1, "cm"),
  title = "foo", grid_width = unit(5, "mm")
)
draw(lgd, test = "grid size")

lgd <- Legend(
  labels = month.name[1:6], legend_gp = gpar(fill = 1:6), title = "foo",
...
lgd <- Legend(
  labels = month.name[1:10], legend_gp = gpar(fill = 1:10), title = "foo",
  ncol = 3
)
...
lgd <- Legend(
  labels = month.name[1:6], legend_gp = gpar(fill = 1:6), title = "foooooo",
...
lgd <- Legend(
  labels = month.name[1:6], title = "foo", type = "points", pch = 1:6,
  legend_gp = gpar(col = 1:6), background = "red"
)
draw(lgd, test = "points as legends")

lgd <- Legend(
  labels = month.name[1:6], title = "foo", type = "points", pch = letters[1:6],
  legend_gp = gpar(col = 1:6), background = "white"
)
draw(lgd, test = "letters as legends")

lgd <- Legend(
  labels = month.name[1:6], title = "foo", type = "lines",
...
lgd <- Legend(
  col_fun = col_fun, title = "foo", at = c(0, 0.5, 1), labels = c("low", "median", "high"),
...
lgd <- Legend(
  col_fun = col_fun, title = "foooooooo", direction = "horizontal",
...
lgd1 <- Legend(
  at = 1:6, legend_gp = gpar(fill = 1:6), title = "legend1",
  nr = 1
)
lgd2 <- Legend(
  col_fun = col_fun, title = "legend2", at = c(0, 0.25, 0.5, 0.75, 1),
  direction = "horizontal"
)
...
lgd <- Legend(
  col_fun = col_fun, title = "foo", at = c(0, 0.1, 0.15, 0.5, 0.9, 0.95, 1),
  direction = "horizontal"
)
draw(lgd, test = "unequal interval breaks")
...
lgd <- Legend(
  col_fun = col_fun, title = "foo", at = c(0, 0.1, 0.5, 0.75, 1),
