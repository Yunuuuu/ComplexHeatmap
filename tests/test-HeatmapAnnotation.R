...
ha <- HeatmapAnnotation(
  foo = 1:10, bar = sample(c("a", "b"), 10, replace = TRUE),
...
ha <- HeatmapAnnotation(foo = function(index) {
  grid.rect()
}, bar = 1:10, height = unit(4, "cm"))
...
ht_list <- Heatmap(m, top_annotation = HeatmapAnnotation(foo = 1:10), column_dend_height = unit(4, "cm")) +
  Heatmap(m,
    top_annotation = HeatmapAnnotation(bar = anno_points(1:10)),
...
ha2 <- HeatmapAnnotation(df = data.frame(
  foo1 = 1:10,
  foo2 = 1:10,
  foo4 = 1:10,
  foo5 = 1:10
...
ha1 <- HeatmapAnnotation(
  foo = sample(c("a", "b"), 10, replace = TRUE),
...
h1 <- HeatmapAnnotation(
  "Subtype" = c("Basal", "Her2", "Normal"),
  col = list("Subtype" = subtype_col)
)
h2 <- HeatmapAnnotation(
  "Subtype" = c("Normal", "Normal", "Basal"),
  col = list("Subtype" = subtype_col)
)
