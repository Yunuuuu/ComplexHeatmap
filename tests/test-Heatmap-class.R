...
ht <- Heatmap(mat,
  row_split = data.frame(rep(c("A", "B"), 12), rep(c("C", "D"), each = 12)),
...
ht <- Heatmap(mat,
  column_split = data.frame(rep(c("A", "B"), 12), rep(c("C", "D"), each = 12)),
...
ht <- Heatmap(mat,
  column_split = rep(c("A", "B"), times = c(6, 18)), column_km = 2,
...
ht <- Heatmap(mat,
  rect_gp = gpar(col = "white"), cell_fun = function(j, i, x, y, width, height, fill) grid.text(mat[i, j], x = x, y = y),
