...
#'
#'
#' @param at Break values. If it is not specified, it is inferred from data
#' scale in current viewport.
#' @param labels Corresponding labels.
#' @param labels_rot Rotations of labels.
#' @param gp Graphic parameters.
#' @param side side of the axis of the annotation viewport.
#' @param facing Facing of the axis.
#' @param direction Direction of the axis. Value should be "normal" or
#' "reverse".
#' @param scale The data scale. If it is \code{NULL}, it is inferred from
#' current viewport.
#' @return A \code{\link[grid:grid.grob]{grob}} object.
#' @examples
#'
#' gb <- annotation_axis_grob(
#'   at = 1:5, labels = month.name[1:5], labels_rot = 0,
#'   side = "left", facing = "outside"
#' )
#' grid.newpage()
#' pushViewport(viewport(xscale = c(0, 4), yscale = c(0, 6), width = 0.6, height = 0.6))
...
#'
#' gb <- annotation_axis_grob(
#'   at = 1:3, labels = month.name[1:3], labels_rot = 0,
#'   side = "top", facing = "outside"
#' )
#' grid.newpage()
#' pushViewport(viewport(xscale = c(0, 4), yscale = c(0, 6), width = 0.6, height = 0.6))
...
#'
#' gb <- annotation_axis_grob(
#'   at = 1:3, labels = month.name[1:3], labels_rot = 90,
#'   side = "top", facing = "outside"
#' )
#' grid.newpage()
#' pushViewport(viewport(xscale = c(0, 4), yscale = c(0, 6), width = 0.6, height = 0.6))
...
#'
#' gb <- annotation_axis_grob(
#'   at = 1:3, labels = month.name[1:3], labels_rot = 45,
#'   side = "top", facing = "outside"
#' )
#' grid.newpage()
#' pushViewport(viewport(xscale = c(0, 4), yscale = c(0, 6), width = 0.6, height = 0.6))
...
annotation_axis_grob <- function(at = NULL, labels = at, labels_rot = 0, gp = gpar(),
  ...
  if (side == "left" && facing == "inside") {
    ...
    gl <- gList(
      ...
      textGrob(labels, unit(1, "npc") - unit(2, "mm"), at,
        default.units = "native",
        rot = 0, just = "right", gp = gp
      )
    )
    ...
    gl <- gList(
      ...
      textGrob(labels, at, unit(1, "npc") - unit(2, "mm"),
        default.units = "native",
...
grid.annotation_axis <- function(at = NULL, labels = at, labels_rot = 0, gp = gpar(),
  ...
  grob <- annotation_axis_grob(
    at = at, labels = labels, labels_rot = labels_rot, gp = gp,
