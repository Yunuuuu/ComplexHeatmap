...
make_comb_mat <- function(..., mode = c("distinct", "intersect", "union"),
  ...
  if (length(lt) == 1) {
    ...
    if (length(dim(lt)) == 2) { # a matrix
      ...
      m <- make_comb_mat_from_matrix(lt,
        mode = mode, top_n_sets = top_n_sets,
  ...
  if (is.atomic(lt[[1]])) {
    m <- make_comb_mat_from_matrix(list_to_matrix(lt),
      mode = mode, top_n_sets = top_n_sets,
      min_set_size = min_set_size, universal_set = universal_set, complement_size = complement_size,
      set_on_rows = set_on_rows
    )
  ...
  m <- make_comb_mat_from_list(lt, value_fun,
    mode = mode, top_n_sets = top_n_sets, min_set_size = min_set_size,
...
upset_top_annotation <- function(m,
  ...
  if (set_on_rows) {
    ...
    ha <- HeatmapAnnotation(
      "intersection_size" = anno_barplot(x,
