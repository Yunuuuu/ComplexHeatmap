...
#'
#' \itemize{ \item \code{\link{show,Heatmap-method}}: draw a single heatmap
#' with default parameters \item \code{\link{draw,Heatmap-method}}: draw a
#' single heatmap.  \item \code{+} or \code{\link[=pct_v_pct]{%v%}} append
#' heatmaps and annotations to a list of heatmaps. }
#'
#' The constructor function pretends to be a high-level graphic function
#' because the \code{show} method of the \code{\link{Heatmap-class}} object
#' actually plots the graphics.
#'
...
#' \code{\link[circlize]{colorRamp2}}). If the matrix is continuous, the value
#' can also be a vector of colors so that colors can be interpolated. Pass to
#' \code{\link{ColorMapping}}. For more details and examples, please refer to
#' \url{https://jokergoo.github.io/ComplexHeatmap-reference/book/a-single-heatmap.html#colors}
#' .
#' @param name Name of the heatmap. By default the heatmap name is used as the
#' title of the heatmap legend.
#' @param na_col Color for \code{NA} values.
#' @param rect_gp Graphic parameters for drawing rectangles (for heatmap body).
#' The value should be specified by \code{\link[grid]{gpar}} and \code{fill}
#' parameter is ignored.
#' @param color_space The color space in which colors are interpolated. Only
#' used if \code{matrix} is numeric and \code{col} is a vector of colors. Pass
#' to \code{\link[circlize]{colorRamp2}}.
#' @param border Whether draw border. The value can be logical or a string of
#' color.
#' @param border_gp Graphic parameters for the borders. If you want to set
#' different parameters for different heatmap slices, please consider to use
#' \code{\link{decorate_heatmap_body}}.
#' @param cell_fun Self-defined function to add graphics on each cell. Seven
#' parameters will be passed into this function: \code{j}, \code{i}, \code{x},
#' \code{y}, \code{width}, \code{height}, \code{fill} which are column index,
#' row index in \code{matrix}, coordinate of the cell, the width and height of
#' the cell and the filled color. \code{x}, \code{y}, \code{width} and
#' \code{height} are all \code{\link[grid]{unit}} objects.
#' @param layer_fun Similar as \code{cell_fun}, but is vectorized. Check
#' \url{https://jokergoo.github.io/ComplexHeatmap-reference/book/a-single-heatmap.html#customize-the-heatmap-body}
#' .
#' @param jitter Random shifts added to the matrix. The value can be logical or
#' a single numeric value. It it is \code{TRUE}, random values from uniform
#' distribution between 0 and 1e-10 are generated. If it is a numeric value,
#' the range for the uniform distribution is (0, \code{jitter}). It is mainly
#' to solve the problem of "Error: node stack overflow" when there are too many
#' identical rows/columns for plotting the dendrograms. ADD: From version
#' 2.5.6, the error of node stack overflow has been fixed, now this argument is
#' ignored.
#' @param row_title Title on the row.
#' @param row_title_side Will the title be put on the left or right of the
#' heatmap?
#' @param row_title_gp Graphic parameters for row title.
#' @param row_title_rot Rotation of row title.
#' @param column_title Title on the column.
#' @param column_title_side Will the title be put on the top or bottom of the
#' heatmap?
#' @param column_title_gp Graphic parameters for column title.
#' @param column_title_rot Rotation of column titles.
#' @param cluster_rows If the value is a logical, it controls whether to make
#' cluster on rows. The value can also be a \code{\link[stats]{hclust}} or a
#' \code{\link[stats]{dendrogram}} which already contains clustering. Check
#' \url{https://jokergoo.github.io/ComplexHeatmap-reference/book/a-single-heatmap.html#clustering}
#' .
#' @param cluster_row_slices If rows are split into slices, whether perform
#' clustering on the slice means?
#' @param clustering_distance_rows It can be a pre-defined character which is
#' in ("euclidean", "maximum", "manhattan", "canberra", "binary", "minkowski",
#' "pearson", "spearman", "kendall"). It can also be a function. If the
#' function has one argument, the input argument should be a matrix and the
#' returned value should be a \code{\link[stats]{dist}} object. If the function
#' has two arguments, the input arguments are two vectors and the function
#' calculates distance between these two vectors.
#' @param clustering_method_rows Method to perform hierarchical clustering,
#' pass to \code{\link[stats]{hclust}}.
#' @param row_dend_side Should the row dendrogram be put on the left or right
#' of the heatmap?
#' @param row_dend_width Width of the row dendrogram, should be a
#' \code{\link[grid]{unit}} object.
#' @param show_row_dend Whether show row dendrogram?
#' @param row_dend_gp Graphic parameters for the dendrogram segments. If users
#' already provide a \code{\link[stats]{dendrogram}} object with edges
#' rendered, this argument will be ignored.
#' @param row_dend_reorder Apply reordering on row dendrograms. The value can
#' be a logical value or a vector which contains weight which is used to
#' reorder rows. The reordering is applied by
#' \code{\link[stats]{reorder.dendrogram}}.
#' @param cluster_columns Whether make cluster on columns? Same settings as
#' \code{cluster_rows}.
#' @param cluster_column_slices If columns are split into slices, whether
#' perform clustering on the slice means?
#' @param clustering_distance_columns Same setting as
#' \code{clustering_distance_rows}.
#' @param clustering_method_columns Method to perform hierarchical clustering,
#' pass to \code{\link[stats]{hclust}}.
#' @param column_dend_side Should the column dendrogram be put on the top or
#' bottom of the heatmap?
#' @param column_dend_height height of the column cluster, should be a
#' \code{\link[grid]{unit}} object.
#' @param show_column_dend Whether show column dendrogram?
#' @param column_dend_gp Graphic parameters for dendrogram segments. Same
#' settings as \code{row_dend_gp}.
#' @param column_dend_reorder Apply reordering on column dendrograms. Same
#' settings as \code{row_dend_reorder}.
#' @param row_order Order of rows. Manually setting row order turns off
#' clustering.
#' @param column_order Order of column.
#' @param row_labels Optional row labels which are put as row names in the
#' heatmap.
#' @param row_names_side Should the row names be put on the left or right of
#' the heatmap?
#' @param show_row_names Whether show row names.
#' @param row_names_max_width Maximum width of row names viewport.
#' @param row_names_gp Graphic parameters for row names.
#' @param row_names_rot Rotation of row names.
#' @param row_names_centered Should row names put centered?
#' @param column_labels Optional column labels which are put as column names in
#' the heatmap.
#' @param column_names_side Should the column names be put on the top or bottom
#' of the heatmap?
#' @param column_names_max_height Maximum height of column names viewport.
#' @param show_column_names Whether show column names.
#' @param column_names_gp Graphic parameters for drawing text.
#' @param column_names_rot Rotation of column names.
#' @param column_names_centered Should column names put centered?
#' @param top_annotation A \code{\link{HeatmapAnnotation}} object.
#' @param bottom_annotation A \code{\link{HeatmapAnnotation}} object.
#' @param left_annotation It should be specified by
#' \code{\link{rowAnnotation}}.
#' @param right_annotation it should be specified by
#' \code{\link{rowAnnotation}}.
#' @param km Apply k-means clustering on rows. If the value is larger than 1,
#' the heatmap will be split by rows according to the k-means clustering. For
#' each row slice, hierarchical clustering is still applied with parameters
#' above.
#' @param split A vector or a data frame by which the rows are split. But if
#' \code{cluster_rows} is a clustering object, \code{split} can be a single
#' number indicating to split the dendrogram by \code{\link[stats]{cutree}}.
#' @param row_km Same as \code{km}.
#' @param row_km_repeats Number of k-means runs to get a consensus k-means
#' clustering. Note if \code{row_km_repeats} is set to more than one, the final
#' number of groups might be smaller than \code{row_km}, but this might means
#' the original \code{row_km} is not a good choice.
#' @param row_split Same as \code{split}.
#' @param column_km K-means clustering on columns.
#' @param column_km_repeats Number of k-means runs to get a consensus k-means
#' clustering. Similar as \code{row_km_repeats}.
#' @param column_split Split on columns. For heatmap splitting, please refer to
#' \url{https://jokergoo.github.io/ComplexHeatmap-reference/book/a-single-heatmap.html#heatmap-split}
#' .
#' @param gap Gap between row slices if the heatmap is split by rows. The value
#' should be a \code{\link[grid]{unit}} object.
#' @param row_gap Same as \code{gap}.
#' @param column_gap Gap between column slices.
#' @param show_parent_dend_line When heatmap is split, whether to add a dashed
#' line to mark parent dendrogram and children dendrograms?
#' @param width Width of the heatmap body.
#' @param height Height of the heatmap body.
#' @param heatmap_width Width of the whole heatmap (including heatmap
#' components)
#' @param heatmap_height Height of the whole heatmap (including heatmap
#' components). Check
#' \url{https://jokergoo.github.io/ComplexHeatmap-reference/book/a-single-heatmap.html#size-of-the-heatmap}
#' .
...
#' annotation legends
#' @param show_heatmap_legend Whether show heatmap legend?
#' @param heatmap_legend_param A list contains parameters for the heatmap
#' legends. See \code{\link{color_mapping_legend,ColorMapping-method}} for all
#' available parameters.
#' @param use_raster Whether render the heatmap body as a raster image. It
#' helps to reduce file size when the matrix is huge. If number of rows or
#' columns is more than 2000, it is by default turned on. Note if
#' \code{cell_fun} is set, \code{use_raster} is enforced to be \code{FALSE}.
#' @param raster_device Graphic device which is used to generate the raster
#' image.
#' @param raster_quality A value larger than 1.
#' @param raster_device_param A list of further parameters for the selected
#' graphic device. For raster image support, please check
#' \url{https://jokergoo.github.io/ComplexHeatmap-reference/book/a-single-heatmap.html#heatmap-as-raster-image}
#' .
#' @param raster_resize_mat Whether resize the matrix to let the dimension of
#' the matrix the same as the dimension of the raster image? The value can be
#' logical. If it is \code{TRUE}, \code{\link[base]{mean}} is used to summarize
#' the sub matrix which corresponds to a single pixel. The value can also be a
#' summary function, e.g. \code{\link[base:Extremes]{max}}.
#' @param raster_by_magick Whether to use
#' \code{\link[magick:transform]{image_resize}} to scale the image.
#' @param raster_magick_filter Pass to \code{filter} argument of
#' \code{\link[magick:transform]{image_resize}}. A character scalar and all
#' possible values are in \code{\link[magick:options]{filter_types}}. The
#' default is \code{"Lanczos"}.
#' @param post_fun A function which will be executed after the heatmap list is
#' drawn.
#' @return A \code{\link{Heatmap-class}} object.
...
Heatmap <- function(
    ...
    row_title_rot = switch(row_title_side[1],
      "left" = 90,
      "right" = 270
    ),
  ...
  if (!is.null(column_split)) {
    if (inherits(cluster_columns, c("dendrogram", "hclust"))) {
      ...
      if (identical(cluster_columns, TRUE) &&
  ...
  if (show_row_names) {
    ...
    if (row_names_centered) {
      row_names_anno <- anno_text(row_labels,
        which = "row", gp = row_names_gp, rot = row_names_rot,
        location = 0.5,
        just = "center"
      )
    } else {
  ...
  if (show_column_names) {
    ...
    if (column_names_centered) {
      column_names_anno <- anno_text(column_labels,
        which = "column", gp = column_names_gp, rot = column_names_rot,
        location = 0.5,
        just = "center"
      )
    } else {
...
#' @rdname Heatmap
#' @include AdditiveUnit-class.R
methods::setClass("Heatmap",
  slots = list(
    name = "character",
...
methods::setMethod(
  f = "prepare", signature = "Heatmap",
  function(object, process_rows = TRUE, process_columns = TRUE) {
...
methods::setMethod(
  ...
  definition = function(object, x, direction = c("horizontal", "vertical")) {
    ...
    ht_list <- methods::new("HeatmapList")
