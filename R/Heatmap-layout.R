  ...
  definition = function(object) {
    ...
    if (show_row_dend) {
      ...
      object@layout$graphic_fun_list <- c(object@layout$graphic_fun_list, function(object) {
        ...
        for (i in seq_len(nr_slice)) {
          draw_dend(object,
            k = i, which = "row", y = slice_y[i], height = slice_height[i], just = "top",
    ...
    if (show_column_dend) {
      ...
      object@layout$graphic_fun_list <- c(object@layout$graphic_fun_list, function(object) {
        ...
        for (i in seq_len(nc_slice)) {
          draw_dend(object,
            k = i, which = "column", x = slice_x[i], width = slice_width[i], just = "left",
    ...
    if (!is.null(annotation)) {
      if (length(annotation@anno_list) > 0) {
        ...
        object@layout$graphic_fun_list <- c(object@layout$graphic_fun_list, function(object) {
          for (i in seq_len(nc_slice)) {
            draw_annotation(object,
              k = i, which = "top", x = slice_x[i], width = slice_width[i],
    ...
    if (!is.null(annotation)) {
      if (length(annotation@anno_list) > 0) {
        ...
        object@layout$graphic_fun_list <- c(object@layout$graphic_fun_list, function(object) {
          for (i in seq_len(nc_slice)) {
            draw_annotation(object,
              k = i, which = "bottom", x = slice_x[i], width = slice_width[i],
    ...
    if (!is.null(annotation)) {
      if (length(annotation@anno_list) > 0) {
        ...
        object@layout$graphic_fun_list <- c(object@layout$graphic_fun_list, function(object) {
          for (i in seq_len(nr_slice)) {
            draw_annotation(object,
              k = i, which = "left", y = slice_y[i], height = slice_height[i],
    ...
    if (!is.null(annotation)) {
      if (length(annotation@anno_list) > 0) {
        ...
        object@layout$graphic_fun_list <- c(object@layout$graphic_fun_list, function(object) {
          for (i in seq_len(nr_slice)) {
            draw_annotation(object,
              k = i, which = "right", y = slice_y[i], height = slice_height[i],
...
names(HEATMAP_LAYOUT_COLUMN_COMPONENT) <- c(
  "column_title_top", "column_dend_top", "column_names_top", "column_anno_top",
...
names(HEATMAP_LAYOUT_ROW_COMPONENT) <- c(
  "row_title_left", "row_dend_left", "row_names_left", "row_anno_left",
  "heatmap_body", "row_anno_right", "row_names_right", "row_dend_right", "row_title_right"
)
