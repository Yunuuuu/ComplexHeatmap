...
textbox_grob <- function(text, x = unit(0.5, "npc"), y = unit(0.5, "npc"), just = "centre",
  ...
  gb <- gTree(
    children = gl, cl = "textbox",
    vp = viewport(
      x = vp_x, y = vp_y, just = vp_just, width = unit(w, "mm"),
      height = unit(h, "mm")
    )
  )
  return(gb)
...
#' row indices.
#' @param text The corresponding texts. The value should be a list of texts. To
#' control graphics parameters of texts in the boxes, The value of \code{text}
#' can also be set as a list of data frames where the first column contains the
#' text, from the second column contains graphics parameters for each text. The
#' column names should be "col", "fontsize", "fontfamily" and "fontface".
#' @param background_gp Graphics for the background.
#' @param which Only "row" is allowed.
#' @param by Are text boxed arranged by \code{\link{anno_link}} or by
#' \code{\link{anno_block}}?
...
anno_textbox <- function(align_to, text, background_gp = gpar(fill = "#DDDDDD", col = "#AAAAAA"),
  ...
  gbl <- lapply(text, function(x) {
    if (is.atomic(x)) {
      ...
      textbox_grob(
        text = x[[1]], gp = gpar(col = col, fontsize = fontsize, fontfamily = fontfamily, fontface = fontface),
  ...
  if (by %in% c("anno_link", "anno_zoom")) {
    ...
    anno <- anno_link(
      align_to = align_to, which = "row", panel_fun = panel_fun,
