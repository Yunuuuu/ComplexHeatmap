...
setMethod(
  ...
  definition = function(object, index, k = 1, n = 1, test = FALSE,
    ...
    pushViewport(viewport(
      width = anno_width, height = anno_height,
    ...
    if (draw_name) {
      ...
      if (is_matrix_annotation(object)) {
        if (!is.null(attr(object@is_anno_matrix, "column_names"))) {
          ...
          grid.text(anno_mat_column_names,
            x = object@name_param$x, y = object@name_param$y, just = object@name_param$just,
            rot = object@name_param$rot, gp = object@name_param$gp
          )
        } else {
          if (object@which == "column") {
            grid.text(object@label,
              x = object@name_param$x[1], y = unit(0.5, "npc"), just = object@name_param$just,
              rot = object@name_param$rot, gp = object@name_param$gp
            )
          } else {
            grid.text(object@label,
              x = unit(0.5, "npc"), y = object@name_param$y[1], just = object@name_param$just,
              rot = object@name_param$rot, gp = object@name_param$gp
            )
          }
        }
