...
oncoPrint <- function(mat, name,
  ...
  if (show_pct) {
    if (pct_side == "left") {
      pct_ha <- rowAnnotation(
        pct = anno_text(pct, just = "right", location = unit(1, "npc"), gp = pct_gp, width = max_text_width(pct, gp = pct_gp) + unit(1, "mm")),
        show_annotation_name = FALSE
      )
    } else {
      pct_ha <- rowAnnotation(
        pct = anno_text(pct, just = "left", location = unit(0, "npc"), gp = pct_gp, width = max_text_width(pct, gp = pct_gp) + unit(1, "mm")),
        show_annotation_name = FALSE
      )
    }
  ...
  if (show_row_names) {
    if (row_names_side == "right") {
      rn_ha <- rowAnnotation(
        rownames = anno_text(row_labels, gp = row_names_gp, just = "left", location = unit(0, "npc"), width = max_text_width(row_labels, gp = row_names_gp) + unit(1, "mm")),
        show_annotation_name = FALSE
      )
    } else {
      rn_ha <- rowAnnotation(
        rownames = anno_text(row_labels, gp = row_names_gp, just = "right", location = unit(1, "npc"), width = max_text_width(row_labels, gp = row_names_gp) + unit(1, "mm")),
        show_annotation_name = FALSE
      )
    }
...
#' @param horiz_margin Horizontal margin. E.g. if you want 1mm margin on top
#' and 1mm margin at bottom of the rectangle, set this value to \code{unit(1,
#' 'mm')}.
#' @param vertical_margin Vertical margin.
#' @param fill Filled color.
...
#' @param beside Will bars be stacked or be positioned beside each other?
#' @param ylim Data range.
#' @param show_fraction Whether to show the numbers or the fractions?
#' @param axis Whether draw axis?
#' @param axis_param Parameters for controlling axis.
...
anno_oncoprint_barplot <- function(type = NULL, which = c("column", "row"),
  ...
  row_fun <- function(index, k, n) {
    ...
    col <- col[!is.na(col)]
    fun <- anno_barplot(v,
      gp = gpar(fill = col, col = NA), which = "row", ylim = ylim,
