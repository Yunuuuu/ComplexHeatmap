...
#' @param levels Levels that correspond to \code{colors}. If \code{colors} is
#' name indexed, \code{levels} can be ignored.
#' @param col_fun Color mapping function that maps continuous values to colors.
#' @param breaks Breaks for the continuous color mapping. If \code{col_fun} is
#' generated by \code{\link[circlize]{colorRamp2}}, \code{breaks} is
#' automatically inferred from the color mapping function.
  ...
  definition = function(object,
    ...
    if (color_bar == "discrete") {
      ...
      if (length(at) == 0) {
        ...
        gf <- Legend(
          at = "NA", labels = "NA", name = object@name, title = title, title_gp = title_gp, grid_height = grid_height,
        ...
        gf <- Legend(
          at = at, labels = labels, name = object@name, title = title, title_gp = title_gp, grid_height = grid_height,
          grid_width = grid_width, tick_length = tick_length, border = border, labels_gp = labels_gp, direction = legend_direction, nrow = nrow, ncol = ncol,
          legend_gp = legend_gp, title_position = title_position, by_row = by_row, graphics = graphics, break_dist = break_dist
        )
      }
      ...
      gf <- Legend(
        at = at, labels = labels, name = object@name, col_fun = object@col_fun, title = title, title_gp = title_gp, grid_height = grid_height,
