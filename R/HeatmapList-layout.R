methods::setMethod(
  f = "make_layout", signature = "HeatmapList",
  function(object,
    ...
    if (direction == "horizontal") {
      ...
      for (obj_nm in c(
        "column_split", "column_km", "column_gap", "cluster_columns", "show_column_dend", "clustering_distance_columns",
      ...
      for (obj_nm in c(
        "row_split", "row_km", "row_gap", "cluster_rows", "show_row_dend", "clustering_distance_rows",
    ...
    no_annotations <- object@ht_list[vapply(object@ht_list, function(x) {
      inherits(x, "Heatmap")
    }, logical(1L))]
    heatmap_legend_list <- c(
      unlist(lapply(no_annotations, function(x) {
...
names(HEATMAP_LIST_LAYOUT_COLUMN_COMPONENT) <- c(
  "annotation_legend_top", "heatmap_legend_top", "column_title_top",
  "heatmap_list", "column_title_bottom", "heatmap_legend_bottom", "annotation_legend_bottom"
)
HEATMAP_LIST_LAYOUT_ROW_COMPONENT <- 1:7
names(HEATMAP_LIST_LAYOUT_ROW_COMPONENT) <- c(
  "annotation_legend_left", "heatmap_legend_left", "row_title_left",
