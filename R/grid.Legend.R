...
Legend <- function(at, labels = at, col_fun, name = NULL, grob = NULL,
  ...
  if (!is.null(grob)) {
    ...
    legend_body <- discrete_legend_body(
    ...
    if (direction == "vertical") {
      legend_body <- vertical_continuous_legend_body(
        at = at, labels = labels, col_fun = col_fun, break_dist = break_dist,
      ...
      legend_body <- horizontal_continuous_legend_body(
        at = at, labels = labels, col_fun = col_fun, break_dist = break_dist,
  ...
  if (title_position %in% c("topleft", "topcenter")) {
    ...
    gf <- grobTree(
      ...
      edit_vp_in_legend_grob(legend_body,
    ...
    if (is.null(graphics)) {
      ...
      if (any(c("points", "p") %in% type)) {
        ...
        if (is.character(pch)) {
          ...
          gl <- c(
            gl,
            .pointsGrob_as_a_list(
              x = grid_x + grid_width[i] * 0.5, y = grid_y, pch = pch[index],
      ...
      if (any(c("boxplot", "box") %in% type)) {
        gl <- c(gl, list(
          segmentsGrob(
            x0 = grid_x + grid_width[i] * 0.5, y0 = grid_y - row_height_no_gap * 0.45,
            x1 = grid_x + grid_width[i] * 0.5, y1 = grid_y + row_height_no_gap * 0.45,
            gp = subset_gp(legend_gp, index)
          ),
          rectGrob(
            x = grid_x + grid_width[i] * 0.5, y = grid_y, width = grid_width[i] * 0.9, height = row_height_no_gap * 0.5,
            gp = subset_gp(legend_gp, index)
          ),
          segmentsGrob(
            x0 = grid_x + grid_width[i] * 0.05, y0 = grid_y,
...
vertical_continuous_legend_body <- function(at, labels = at, col_fun,
  ...
  if (!at_diff_is_equal) {
    labels_line_height <- do.call("unit.c", lapply(
      labels,
    ...
    yrange <- convertY(unit.c(
      offset - labels_line_height[1] * 0.5,
      legend_body_height - offset + labels_line_height[length(labels_line_height)] * 0.5
    ), "mm", valueOnly = TRUE)
  ...
  y2 <- seq(0, 1, length.out = length(colors) + 1)
  y2 <- y2[-length(y2)]
  ...
  gl <- c(gl, list(
    rectGrob(x2, rev(y2),
      width = grid_width, height = hh, just = c("left", "center"),
...
horizontal_continuous_legend_body <- function(at, labels = at, col_fun,
  ...
  labels_width <- do.call("unit.c", lapply(
    labels,
  ...
  x2 <- seq(0, 1, length.out = length(colors) + 1)
  x2 <- x2[-length(x2)] # left of rects
  ...
  gl <- c(gl, list(
    rectGrob(x2, y2,
      height = grid_height, width = ww, just = c("left", "top"),
