...
abs_units <- c(
  "cm", "inches", "mm", "points", "picas", "bigpts", "dida", "cicero",
...
.is_abs_unit.unit.list <- function(x) {
  all(sapply(x, function(y) {
    if (inherits(y, "unit.arithmetic")) {
      .is_abs_unit.unit.arithmetic(y)
    } else if (inherits(y, "unit.list")) {
      .is_abs_unit.unit.list(y)
    } else if (inherits(y, "unit")) {
      .is_abs_unit.unit(y)
    } else {
      FALSE
    }
...
is_abs_unit_v3 <- function(u) {
  if (inherits(u, "unit.arithmetic")) {
    .is_abs_unit.unit.arithmetic(u)
  } else if (inherits(u, "unit.list")) {
    .is_abs_unit.unit.list(u)
