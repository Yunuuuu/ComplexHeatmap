...
#' \code{\link{anno_lines}}, \code{\link{anno_barplot}},
#' \code{\link{anno_boxplot}}, \code{\link{anno_histogram}},
#' \code{\link{anno_density}}, \code{\link{anno_joyplot}},
...
#' consistency of annotations and heatmaps.
#' @param data_scale The data scale on the data axis (y-axis for column
#' annotation and x-axis for row annotation). It is only used when
...
#' and normally they will be drawn beside the annotation graphics to tell what
#' the annotation is about. e.g. the annotation names put beside the points
#' annotation graphics. However, for some of the annotations, the names are not
#' necessarily to be drawn, such as text annotations drawn by
#' \code{\link{anno_text}} or an empty annotation drawn by
#' \code{\link{anno_empty}}. In this case, when \code{show_names} is set to
#' \code{FALSE}, there will be no annotation names drawn for the annotation.
#' @param width The width of the plotting region (the viewport) that the
#' annotation is drawn. If it is a row annotation, the width must be an
#' absolute unit. Since the \code{AnnotationFunction} object is always
#' contained by the \code{\link{SingleAnnotation-class}}object, you can only
#' set the width of row annotations or height of column annotations, while e.g.
#' the height of the row annotation is always \code{unit(1, "npc")} which means
#' it always fully filled in the parent \code{SingleAnnotation} and only in
#' \code{\link{SingleAnnotation}} or even \code{\link{HeatmapAnnotation}} can
#' adjust the height of the row annotations.
#' @param height The height of the plotting region (the viewport) that the
#' annotation is drawn. If it is a column annotation, the width must be an
#' absolute unit.
  ...
  definition = function(object, index, k = 1, n = 1, test = FALSE, ...) {
    ...
    if (test2) {
      ...
      if (!identical(unit(0, "mm"), object@extended[1])) {
        grid.rect(
          y = 1, height = unit(1, "npc") + object@extended[1], just = "top",
          gp = gpar(fill = "transparent", col = "red", lty = 2)
        )
      } else if (!identical(unit(0, "mm"), object@extended[2])) {
        grid.rect(
          x = 1, width = unit(1, "npc") + object@extended[2], just = "right",
          gp = gpar(fill = "transparent", col = "red", lty = 2)
        )
      } else if (!identical(unit(0, "mm"), object@extended[3])) {
        grid.rect(
          y = 0, height = unit(1, "npc") + object@extended[3], just = "bottom",
          gp = gpar(fill = "transparent", col = "red", lty = 2)
        )
      } else if (!identical(unit(0, "mm"), object@extended[4])) {
        grid.rect(
          x = 0, width = unit(1, "npc") + object@extended[4], just = "left",
          gp = gpar(fill = "transparent", col = "red", lty = 2)
        )
      }
    }
