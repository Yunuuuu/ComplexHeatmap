...
#' right if the dendrogram is horizontal and it is put on the top if the
#' dendrogram is vertical.
#' @param gp Graphic parameters for the dendrogram segments. If any of
#' \code{col}, \code{lwd} or \code{lty} is set in the \code{edgePar} attribute
#' of a node, the corresponding value defined in \code{gp} will be overwritten
#' for this node, so \code{gp} is like global graphic parameters for dendrogram
#' segments.
...
dendrogramGrob <- function(
  ...
  if (facing %in% c("bottom", "top")) {
    ...
    if (any(lt$node)) {
      l <- lt$node
      cl[[2]] <- pointsGrob(lt$node_x[l], lt$node_y[l],
        pch = lt$node_pch[l], size = lt$node_size[l],
    ...
    if (any(lt$node)) {
      l <- lt$node
      cl[[2]] <- pointsGrob(lt$node_y[l], lt$node_x[l],
        pch = lt$node_pch[l], size = lt$node_size[l],
        gp = gpar(col = lt$node_col[l], fill = lt$node_fill[l], cex = lt$node_cex[l]),
        default.units = "native"
      )
  ...
  if (test) {
    ...
    if (gb$facing %in% c("top", "bottom")) {
      pushViewport(viewport(
        xscale = c(-0.5, n + 0.5), yscale = c(-h * 0.05, h * 1.05),
        width = unit(1, "npc") - unit(4, "cm"),
        height = unit(1, "npc") - unit(4, "cm")
      ))
    } else {
      pushViewport(viewport(
        yscale = c(-0.5, n + 0.5), xscale = c(-h * 0.05, h * 1.05),
        width = unit(1, "npc") - unit(4, "cm"),
        height = unit(1, "npc") - unit(4, "cm")
      ))
    }
...
dend_edit_node <- function(
    dend, fun = function(d, index) d,
